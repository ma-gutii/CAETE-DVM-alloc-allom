...
# Converta a coluna 'date' para o formato de data sem o dia
table_regclim <- table_regclim %>%
  mutate(date = as.Date(paste(date, "01", sep = "-")),   # Adiciona o dia '01'
         date = substr(date, 1, 7))                      # Remove o dia


# Converter a coluna ws para um fator para garantir cores diferentes
table_regclim$ws <- as.factor(table_regclim$ws)
...
# Criar um gráfico utilizando ggplot2 com a paleta de cores distinta
ggplot(table_regclim, aes(x = timeindex, y = ar1, color = ws)) +
  geom_line() +
  scale_color_manual(values = cores_distintas) +
  labs(title = "AR1 ao longo do Timeindex",
       x = "Timeindex",
