...
for variable in variables_to_plot:
    ...
    fig, axes = plt.subplots(len(locs), 1, figsize=(15, 20), sharex=True)
    ...
    for i, loc in enumerate(locs):
        ...
        axes[i].plot(df_loc_var['date_dateformat'], df_loc_var[variable], linewidth=0.7, alpha=1, label=f'{loc}')

        axes[i].set_ylabel(f'{variable.upper()}', fontsize=14)  # Ajustar o tamanho da fonte no eixo y
        ...
        axes[i].legend(fontsize=10)  # Ajustar o tamanho da fonte na legenda

        # Ajustar o tamanho dos valores dos eixos x e y
        axes[i].tick_params(axis='both', labelsize=14)

    # Adicionar rótulo para o eixo x comum e ajuste manual do layout
    axes[-1].set_xlabel('Date', fontsize=14)  # Ajustar o tamanho da fonte no eixo x
    plt.suptitle(f'Time series - {variable} precipitation reduction', y=1.02, fontsize=16)  # Ajustar o tamanho da fonte no título
