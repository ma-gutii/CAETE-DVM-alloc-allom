...
class grd:
    ...
    def init_caete_dyn(self, input_fpath, stime_i, co2, pls_table, tsoil, ssoil, hsoil):
        ...
        self.vp_croot = np.zeros(shape=(npls,), order='F') + 0.1#+ 1.0
        self.vp_cwood = np.zeros(shape=(npls,), order='F') + 1. #+ 0.1
        ...
        for s in range(spin):
            ...
            for step in range(steps.size):
                ...
                if self.vp_lsid.size < 1 and not save:
                    ...
                    self.vp_cleaf = np.zeros(shape=(self.vp_lsid.size,)) + 0.1
                    self.vp_cwood = np.zeros(shape=(self.vp_lsid.size,))
                    self.vp_croot = np.zeros(shape=(self.vp_lsid.size,)) + 0.1
